...
#[repr(transparent)]
pub struct Sha3<const S: usize> {
    state: cx_sha3_t,
}

impl<const S: usize> Sha3<S> {
    pub const DIGEST_LEN: usize = S;

    #[inline(never)]
    pub fn new() -> Result<Self, Error> {
        zemu_sys::zemu_log_stack("Sha3::new\x00");
        let mut this = Self {
    ...
    fn init_state(state: *mut cx_sha3_t) -> Result<(), Error> {
        cfg_if! {
            if #[cfg(bolos_sdk)] {
                match unsafe { crate::raw::cx_sha3_init_no_throw(state, S * 8) } {
                    0 => {},
                    err => return Err(err.into()),
                }
            } else {
                unimplemented!("sha3 init called in non bolos")
            }
        }
...
impl<const S: usize> CxHash<S> for Sha3<S> {
