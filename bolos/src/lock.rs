...
********************************************************************************/
/// This structure is a utility to prevent concurrent use of a certain resource
/// in the context of an app.
///
/// The app won't be run multithreaded, but we use resources (or global state) to
/// store data between calls to the app. The aim of [`Lock`] is to enforce that
/// a certain resource is not used by different instructions accidentally.
///
/// Permission control (limiting who can access the resource) is done via `A`
/// which is typically an enum which lists the different "accessors" of the resource
pub struct Lock<T, A> {
    item: T,
    lock: Option<A>,
}
...
impl<T, A: Eq> Lock<T, A> {
    /// Locks the resource, marking it with the given `acquirer` and returns it
    ///
    /// Will forcefully take ownership of the lock
